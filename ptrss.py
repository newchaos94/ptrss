...
class RssDB():
    ...
    def init_db(self):
        ...
        cnt = self.csr.execute("select count(*) from sqlite_master where type='table' and name = 'torrents';")
        if cnt == 0:
...
class Rss():
    ...
    def __init__(self, config):
        ...
        if self.config.get('download') is None:
    ...
    def _request(self, url):
        ...
        if resp.status_code == 302:
            raise SyntaxError("网站无法登陆, 更新cookie~~")
        return resp


    def _info(self, entry):
        print('----------------{}----------------'.format(entry.title))

        eid = entry.id
        if 1 == rssdb.scale(eid):
            raise RuntimeError()

        # 获取文件大小 GB 保留1位小数
        size = round(int(entry.links[1]['length']) / self.UNIT, 1)
        ...
        rssdb.insert((eid, entry.title, size, discount, time.time()))
        return size, discount

    def _check(self, entry):
        try:
            size, discount = self._info(entry)
            print(size, discount)
